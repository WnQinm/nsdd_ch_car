        ...
        if(left_circle_flag)
        {
            switch (circle_status)
            {
                case 1:// step1 避开第一个断口(正常巡线应该就行)
                    ...
                    if(++cnt>1000 && adc_LL>circle_threshold)// && current_err_circle>current_err_common
                    {
                        circle_status++;
                        cnt%=1000;
                    }
        ...
        else
        {
            switch (circle_status)
            {
                case 1:// step1 避开第一个断口(正常巡线应该就行)
                    ...
                    if(++cnt>1000 && adc_RR>circle_threshold)// && current_err_circle>current_err_common
                    {
                        circle_status++;
                        cnt%=1000;
                    }
                ...
                case 5:// step5 出环后
                    CURRENT_STATUS = Status_Common;
                    if(++cnt>2000 && adc_RR<circle_threshold)
                    {
                        cnt%=2000;
                        circle_status = 0;
                        right_circle_flag = false;
