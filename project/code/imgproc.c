...
void AddLine()
{
    ...
    {
        if(LeftBreakpoint.start_y==-1 && leftline[row]-leftline[row-2]>20)//左边线下减上>20，下高上低，左下角点
            LeftBreakpoint.start_y = row;
        else if(LeftBreakpoint.end_y!=-1 && leftline[row-2]-leftline[row]>20)//左边线上减下>20，上高下低，左上角点
            LeftBreakpoint.end_y = row;

        if(RightBreakpoint.start_y==-1 && rightline[row-2]-rightline[row]>20)//右边线上减下>20，上低下高，右下角点
            RightBreakpoint.start_y = row;
        else if(RightBreakpoint.end_y!=-1 && rightline[row]-rightline[row-2]>20)//右边线下减上>20，下低上高，右上角点
            RightBreakpoint.end_y = row;
    }

    // 对左边线：
    if(LeftBreakpoint.start_y!=-1 && LeftBreakpoint.end_y!=-1)// 上下角点都有
        CommonRectificate(leftline, LeftBreakpoint.end_y, LeftBreakpoint.start_y);//end在上面start在下面，所以begin是end
    else if(LeftBreakpoint.start_y!=-1 && LeftBreakpoint.end_y==-1)// 只有下角点没有上角点
    {
    ...
    else if(LeftBreakpoint.start_y==-1 && LeftBreakpoint.end_y!=-1)// 只有上角点没有下角点
    {
    ...
    // 对右边线：
    if(RightBreakpoint.start_y!=-1 && RightBreakpoint.end_y!=-1)// 上下角点都有
        CommonRectificate(rightline, RightBreakpoint.end_y, RightBreakpoint.start_y);
    else if(RightBreakpoint.start_y!=-1 && RightBreakpoint.end_y==-1)// 只有下角点没有上角点
    {
    ...
    else if(RightBreakpoint.start_y==-1 && RightBreakpoint.end_y!=-1)// 只有上角点没有下角点
    {
